<%@ Page Title="Tương tác" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Contact.aspx.cs" Inherits="webtintuc.Contact" %>

<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <h2><%: Title %></h2>
    <h3>Chi tiết liên hệ với tôi!</h3>
    <address>
        Ngô Việt Hải<br/>
        CTT11CD3A<br/>
        Khoa Điện-Điện tử<br/>
        Gmail: ngoviethai55kttt@gmail.com<br/>
        Trường cao đẳng KTCN Nha Trang<br/>
        <abbr title="Phone"></abbr>
        0343191276
    </address>
    <address>
        <strong>Facebook:</strong><a href="https://www.facebook.com/ngoviethai55kttt">My facebook</a><br />
        <strong>Youtube:</strong> <a href="https://www.youtube.com/channel/UCI8rC_86GHb9RXPCBzmPO4w?view_as=subscriber">My channel</a>
    </address>
</asp:Content>
